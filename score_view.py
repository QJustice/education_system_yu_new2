...
class ScoreVis:
    def __init__(self):
        ...
        # 建立数据库连接
        self.condaba = ConnectDatabase.ConData(_host='127.0.0.1',
                                               _db='student2022',
                                               _port=3306,
                                               _user='root',
                                               _password='root',
                                               _charset='utf8')
        # 获取数据库连接
        conn = self.condaba.get_con_database()
        self.cur = conn.cursor()

    def student_score_view(self, _lenName):
        lesName = _lenName
        # 用于数据库查询的语句
    ...
    def score_view(self):
        ...
        self.cur.execute(sqlcmdall)
        tempall = self.cur.fetchall()
        self.cur.execute(sqlcmdAvg)
        tempAvg = self.cur.fetchall()
        # print(tempAvg)
    ...
    def student_pass_rate(self, _lenName):
        lesName = _lenName
        ...
        sqlcmdall = "SELECT studentinfo.StudentNo, studentinfo.`Name`,lessoninfo.lesName,score " \
                    ...
                    "AND stchoose.lesNo = lessoninfo.lesNo " \
                    "AND lessoninfo.lesName = '%s'" % lesName
        self.cur.execute(sqlcmdallok)
        tempallok = self.cur.fetchall()
        # print(tempallok)
        # print(len(tempallok))
        self.cur.execute(sqlcmdall)
        tempall = self.cur.fetchall()
        ...
        # print(len(tempall))
        langs = ['及格', '不及格']
        data = [float(len(tempallok)) / float(len(tempall)), float(len(tempall) - len(tempallok)) / float(len(tempall))]
        pyplot.title("%s课程及格率" % lesName)
        pyplot.pie(data, labels=langs, autopct='%1.2f%%')
        pyplot.show()

    def less_view(self, _lessName):
        sqlcmdAvg = "SELECT lessoninfo.lesName,AVG( score )" \
                    "FROM stchoose,lessoninfo " \
                    "WHERE lessoninfo.lesNo = stchoose.lesNo " \
...
if __name__ == '__main__':
    test = ScoreVis()
    test.score_view()
    test.student_score_view("Math")
    test.student_pass_rate("English")
    test.less_view("Python")
