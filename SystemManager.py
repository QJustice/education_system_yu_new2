...
class SystemManager:
    ...
    def RegTeacher(self):
        ...
        salary = input('         Salary:')
        sqlcmd = "insert into TeacherInfo(Name,TeacherNo,Gender,Birth,PositionNo,Salary) values('%s','%s','%s','%s',%s,%s)" % (
    ...
    def ChangeTeacherInfo(self):
        ...
        if res == 0:
            ...
            position = input('Position Number:')
            salary = input('         Salary:')
            sqlcmd = "update TeacherInfo Set Name='%s',TeacherNo='%s',Gender='%s',Birth='%s',PositionNo='%s',Salary='%s' where TeacherNo = '%s'" % (
    ...
    def grade_view(self):
        plt.rcParams['font.sans-serif'] = ['SimHei']  # 汉字显示正常
        plt.rcParams['axes.unicode_minus'] = False  # 负号的正常显示
        cur = self.conn.cursor()
        cur.execute('select StudentNo,Grade from Studentinfo')
        tp = cur.fetchall()
        plt.title('学生分数展示')
        plt.xlabel('学号')
        plt.ylabel('分数')
        ...
        for item in tp:
            ...
            plt.bar(item[0], int(item[1]))
            plt.text(item[0], int(item[1]), int(item[1]), ha='center', va='bottom', fontsize=10)
        plt.show()
...
if __name__ == '__main__':
    ...
    sm = SystemManager(conn, '20022', '123456')
    sm.MainFunc()
    conn.close()
